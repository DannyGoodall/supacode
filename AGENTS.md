...
## Jujutsu (jj) co-located integration (experimental)

- Gated behind `@Shared(.experimentalJJIntegration)` (`Features/Settings/Shared/ExperimentalSettingsKeys.swift`, default `false`). With the gate **off**, behavior is byte-for-byte identical to before — the colocation probe is skipped and every repo classifies `.git`/`.folder` as it always did. Only **co-located** repos are in scope: `.jj` and `.git` as peers in the repo root (jj's term). Non-colocated jj (no sibling `.git`) is explicitly unsupported.
- `Repository.vcs: RepositoryVCS` (`.git` / `.gitColocatedJJ` / `.folder`) replaced the old stored `isGitRepository: Bool`; `isGitRepository` is now a computed `vcs != .folder`, and the historical `init(isGitRepository:)` is preserved — so the ~50 existing `isGitRepository` consumers are untouched. `isColocatedJJ` is `vcs == .gitColocatedJJ`. The loader (`RepositoriesFeature.loadRepositoriesData` → `classifyRoot`) only promotes a root to `.gitColocatedJJ` when the gate is on and `Repository.isColocatedJJRepository(at:)` (a pure FileManager probe) passes.
- Backend selection is per-repo: `Repository.usesJujutsuBackend(vcs:preferJJ:)` = `vcs == .gitColocatedJJ && (preferJJ ?? true)`. `RepositorySettings.preferJJ: Bool?` is a persisted tri-state — `nil` = use the default (prefer jj for a colocated repo), `true`/`false` = explicit per-repo override (set via the "Version Control" picker in `RepositorySettingsView`, only shown when `isColocatedJJ`).
- **No new protocol/DI seam.** Dispatch happens inside the existing `GitClientDependency` closures: each live closure calls `GitClientDependency.shouldUseJujutsuBackend(for: repoRoot)` (reads gate + per-repo `preferJJ` + filesystem colocation probe) and routes to `JJClient` (jj) or `GitClient` (git, literally unchanged). Reducers/call sites/tests depend on `GitClientDependency`, not a backend type, so they're untouched. The user-facing vocabulary stays worktree/branch/new/delete regardless of backend.
- `JJClient` (`Clients/Jujutsu/JJClient.swift`) is the jj backend. Mapping: worktree→workspace (`jj workspace add/forget`, enumerated live via `jj workspace list` + `jj workspace root --name` — discovers pre-existing/external workspaces, no registry), branch→bookmark (`jj bookmark …`; creating a workspace auto-creates a bookmark at its `@` so the "branch" is non-anonymous for PR/push), base ref→revset (git-style `remote/branch` → `branch@remote` only when the prefix is a known remote), fetch→`jj git fetch`, push→`jj git push --bookmark <n> --allow-new`, diff→`jj diff --stat -r @`. **Every read op passes `--ignore-working-copy`** so watcher-triggered reads can't auto-snapshot.
- Push surface is **CLI + deeplink only** (no toolbar action): `supacode worktree push` / `supacode://worktree/<id>/push` → `RepositoriesFeature.pushWorktreeBookmark` → routed `GitClientDependency.pushBranch` (jj `jj git push --bookmark`; git `git push -u origin`). Failures surface via `presentAlert` (so the CLI socket response reports the error); silent success = CLI ok.
- Watcher: `WorktreeInfoWatcherManager.watchURL(for:)` routes to `JJWorktreeStateResolver.opHeadsURL` (the repository's `.jj/repo/op_heads/heads` dir) for jj backends, else `GitWorktreeHeadResolver.headURL` (`.git/HEAD`). The op-log head changes only on real jj operations, never on `--ignore-working-copy` reads, so it can't feed back into the watcher. Same DispatchSource/debounce/emit pipeline; the branch label re-derives downstream via the jj-routed `gitClient.branchName`.
- **Degradation:** any jj failure (CLI missing/errored) in the `worktrees` closure falls back to the Git backend so a colocated repo never fails to load.
- **Dev/GUI testing:** a Debug build collides with the installed release app via shared UserDefaults (bundle id), `~/.supacode` data dir, and the `$TMPDIR/zmx-<uid>` socket dir — the zmx collision will disrupt/duplicate live terminal sessions. Debug builds are isolated (bundle id `…​.debug`, `~/.supacode-debug` data dir, `ZMX_DIR=/tmp/zmx-<uid>-dbg`); even so, prefer running only one supacode at a time when testing.
- **Build:** Zig 0.15.2 can't link the macOS 26.4+ SDK, so build with Xcode ≤ 26.3 (`export DEVELOPER_DIR=/Applications/Xcode-26.3.0.app/Contents/Developer`).
