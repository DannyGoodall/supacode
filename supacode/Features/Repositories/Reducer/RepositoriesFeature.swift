...
struct RepositoriesFeature {
  ...
  var worktreeArchiveReducer: some Reducer<State, Action> {
    Reduce { state, action in
      ...
      case .requestDeleteSidebarItems(let targets):
        ...
        let bmNounPlural = deleteVocab.bookmarkNounPlural.lowercased()
        ...
        let message: String =
          ...
          case (_, true, false):
            "This deletes \(count) \(wsNoun) directories and their local \(bmNounPlural)."
          case (_, false, false):
            "This deletes \(count) \(wsNoun) directories but keeps their local \(bmNounPlural)."
          }
        state.alert = AlertState {
          TextState(title)
        } actions: {
  ...
  var body: some Reducer<State, Action> {
    Reduce { state, action in
      ...
      case .openRepositoriesFinished(let repositories, let failures, let invalidRoots, let roots):
        ...
        // refresh / reload doesn't hijack the user's current selection. Gated on
        // a non-empty prior set so the INITIAL load (and remote resolution,
        // which routes through here) keeps upstream's selection semantics —
        // only a genuine add to an existing workspace opens the new repo.
        if !previousRoots.isEmpty,
          let newRoot = roots.first(where: { !previousRoots.contains($0.standardizedFileURL) }),
      ...
      case .pinWorktree, .unpinWorktree, .pushWorktreeBookmark, .presentAlert, .showToast, .dismissToast,
        .delayedPullRequestRefresh, .worktreeNotificationReceived, .worktreeInfoEvent:
  ...
  private struct WorktreesFetchResult: Sendable {
    ...
    /// Only ever `true` when the experimental jj integration is on AND the root
    /// is a colocated git+jj repo. Drives the additive `isColocatedJJ` flag on
    /// `Repository`; otherwise the repo stays plain `.git`.
  ...
  nonisolated private static func worktreesFetchResult(
    ...
    guard isGit else {
      return WorktreesFetchResult(
        root: root, isGitRepository: false, isColocatedJJ: false, worktrees: [], errorMessage: nil)
    }
    // Cheap filesystem probe, skipped entirely when the experimental setting is
  ...
  private func loadRepositoriesData(_ roots: [URL]) async -> ([Repository], [LoadFailure]) {
    ...
    // historical git/folder behavior.
    ...
    let fetchResults = await withTaskGroup(of: WorktreesFetchResult.self) { group in
      for root in roots {
        ...
        group.addTask {
          await Self.worktreesFetchResult(
            for: root, jjIntegrationEnabled: jjIntegrationEnabled, gitClient: gitClient)
        }
      }
      ...
      if result.isGitRepository {
        if let worktrees = result.worktrees {
          let repository = Repository(
            ...
            isGitRepository: true,
            isColocatedJJ: result.isColocatedJJ
          )
...
extension RepositoriesFeature.State {
  ...
  /// present jj-native vocabulary). A repo is only `isColocatedJJ` when the
  /// experimental gate is on, so this just layers the per-repo `preferJJ`
  /// override on top of that flag — matching
  /// `GitClientDependency.shouldUseJujutsuBackend(for:)`.
  func usesJujutsuBackend(forRepository id: Repository.ID) -> Bool {
    guard let repository = repositories[id: id], repository.isColocatedJJ else {
      return false
    }
    @SharedReader(.repositorySettings(repository.rootURL)) var settings
    return Repository.usesJujutsuBackend(isColocatedJJ: repository.isColocatedJJ, preferJJ: settings.preferJJ)
  }
